...
//!
//! The narrowed-trait types introduced by ADR-027 (`SshTarget`, `Liveness`,
//! universal probe) live canonically at [`crate::provider::target`]. A few
//! of those names are re-exported here for ergonomic access from
//! `exedev`-flavoured call sites.
...
use crate::provider::{RemoteInstance, RemoteProvider, target};

pub use crate::provider::target::{DEFAULT_PROBE_TIMEOUT, Liveness, SshTarget};
