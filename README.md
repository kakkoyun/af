...
| Command                                                                                                              | Description                                                                                                                      |
| -------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------- |
| `af create [name] [--from BRANCH] [--current] [--agent NAME] [--bare] [--no-attach] [--remote HOST] [--sandbox PROVIDER] [--yolo]` | Create a workstream: new branch, git worktree, `state.toml`, ledger, optional Obsidian note, tmux session, primary-agent launch. When run interactively it then attaches to that tmux session; `--no-attach` (implied by `--bare`) prints a next-steps hint instead. |
...
| `af resume [session] [--bare]`                                                                                       | Resume a suspended workstream (respawning its tmux session if it died) and attach. On an already-active workstream, attaches instead of erroring; `--bare` skips the respawn/attach and prints a manual-attach hint. |
