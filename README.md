...
| Command                                                                                                              | Description                                                                                                                      |
| -------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------- |
| `af create [name] [--from BRANCH] [--current] [--agent NAME] [--bare] [--no-attach] [--remote HOST] [--sandbox PROVIDER] [--yolo]` | Create a workstream: new branch, git worktree, `state.toml`, ledger, optional Obsidian note, tmux session, primary-agent launch. With `--sandbox`, the host tmux pane is a plain shell into the sandbox VM instead (`slicer vm shell <vm>`) — the agent launches inside the VM only, not on the host. When run interactively it then attaches to that tmux session, switching the client instead of opening a new terminal when already inside tmux; `--no-attach` (implied by `--bare`) prints a next-steps hint instead. |
| `af done [session] [--force] [--discard]`                                                                            | Tear down and archive a workstream. `--force` marks it abandoned; `--discard` skips the automatic transcript sync.      |
...
| `af resume [session] [--bare]`                                                                                       | Resume a suspended workstream (respawning its tmux session if it died) and attach. On an already-active workstream, respawns a dead tmux session (if the server or session died out from under it) before attaching instead of erroring; either way attach switches the client instead of opening a new terminal when already inside tmux; `--bare` skips the respawn/attach and prints a manual-attach hint. |
