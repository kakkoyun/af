...
// SBXConfig contains sbx-specific sandbox defaults.
...
func parseSandboxSection(table map[string]any, path string, _ bool, _ *slog.Logger, layer *configLayer) error {
	...
	err = parseSlicerSection(table, path, layer)
	if err != nil {
		return err
	}
	err = parseSBXSection(table, path, layer)
	if err != nil {
		return err
	}
...
func parseSBXSection(table map[string]any, path string, layer *configLayer) error {
	sbx, ok, err := optionalTable(table, "sbx", path)
	if err != nil || !ok {
		return err
	}
	layer.Sandbox.SBXImage, err = stringPointer(sbx, "image", path)
	if err != nil {
		return err
	}
...
func mergeSandbox(cfg *SandboxConfig, layer sandboxLayer) {
	...
	assignString(&cfg.SBX.Image, layer.SBXImage)
}
